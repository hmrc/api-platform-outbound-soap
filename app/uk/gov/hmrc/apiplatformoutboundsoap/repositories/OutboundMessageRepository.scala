...
import uk.gov.hmrc.apiplatformoutboundsoap.models.{OutboundSoapMessage, RetryingOutboundSoapMessage, DeliveryStatus}
...
class OutboundMessageRepository @Inject()(mongoComponent: ReactiveMongoComponent, appConfig: AppConfig)
  ...
  def retrieveMessagesForRetry: Source[RetryingOutboundSoapMessage, Future[Any]] = {
    ...
    collection
      .find(Json.obj("status" -> DeliveryStatus.RETRYING.entryName,
  ...
  def updateSendingStatus(globalId: UUID, newStatus: DeliveryStatus): Future[Option[OutboundSoapMessage]] = {
    findAndUpdate(Json.obj("globalId" -> globalId),
      Json.obj("$set" -> Json.obj("status" -> newStatus.entryName)), fetchNewObject = true)
      .map(_.result[OutboundSoapMessage])
  }

  def updateConfirmationStatus(globalId: UUID, newStatus: DeliveryStatus, confirmationMsg: String): Future[Option[OutboundSoapMessage]] = {
