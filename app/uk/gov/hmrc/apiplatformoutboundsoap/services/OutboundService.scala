...
class OutboundService @Inject() (
  ...
  private def parseWsdl(wsdlUrl: String): Future[Definition]                                          = {
    Future.successful(wsdlReader.readWSDL(wsdlUrl))
  }
  ...
  private def addWithAddressElementChildToSoapHeader(isRequired: String => Boolean, property: String, elementName: String, namespace: OMNamespace, envelope: SOAPEnvelope): Unit = {
